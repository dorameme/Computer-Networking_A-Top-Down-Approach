...
## HTTP/1.1에서의 해결 방법
### TCP 혼잡 제어
- HTTP/1.1은 TCP 혼잡 제어를 통해 병목 링크의 대역폭을 여러 TCP 연결이 공평하게 나누어 사용하도록 한다.
- 여러 병렬 연결을 통해 대역폭을 분배하며, 각 연결은 대역폭의 1/n씩 사용하게 된다.

## HTTP/2에서의 해결 방법
### 프레이밍
- HTTP/2는 HTTP 메시지를 독립된 프레임으로 쪼개고, 이를 인터리빙하여 전송하며, 반대편 사이트에서 재조립하는 방식이다.
- 이를 통해 HOL 블로킹 문제를 피할 수 있다. HTTP/2는 여러 병렬 TCP 연결을 줄이고, 하나의 TCP 연결로 여러 요청을 처리할 수 있도록 프레이밍 방식을 사용한다.

### 메시지 우선순위화
- HTTP/2는 각 메시지에 우선순위를 부여하여 서버가 가장 중요한 요청을 먼저 처리할 수 있도록 한다.
- 이를 통해 클라이언트와 서버 간의 효율적인 데이터 처리 순서를 제어할 수 있다.

### 서버 푸싱
- HTTP/2는 서버 푸싱 기능을 통해 서버가 클라이언트의 요청 없이도 필요한 객체를 미리 전송할 수 있다. 
- 이를 통해 웹 페이지를 완전하게 구성하는 데 필요한 객체들을 클라이언트에게 지연 없이 전송할 수 있다.

## HTTP/3에서의 해결 방법
- HTTP/3는 QUIC 프로토콜을 기반으로 설계된 HTTP 프로토콜로, 표준화는 아직 진행 중이다.
- QUIC는 UDP 위에서 동작하며, 더 빠르고 효율적인 연결을 제공한다.
