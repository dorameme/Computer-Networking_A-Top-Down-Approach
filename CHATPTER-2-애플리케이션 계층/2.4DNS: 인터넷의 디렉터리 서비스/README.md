...
## DNS 취약점
### DDoS 대역폭 플러딩
- **목적**: 다량의 패킷을 **DNS 서버**에 보내 대역폭을 초과하게 만드는 공격이다.
- **기법**: 대량의 요청을 보내 DNS 서버가 정상적인 요청을 처리하지 못하도록 하는 방법이다.
- **예시**: 대량의 패킷을 보내는 시도가 있었으나, **ICMP 핑 메시지**를 차단함으로써 **미미한 효과**만을 가져왔다.    
     하지만 최상위 도메인 서버(TLD 서버)에 다량의 DNS 질의를 보내는 방식은 여전히 효과적인 **DDoS 공격**이다.
  - 이유는 최상위 도메인 서버가 **광범위한 질의**를 처리해야 하므로 대역폭이 금방 초과되기 때문이다.

#### 중간자 공격 (Man-in-the-Middle Attack)
- **목적**: 공격자가 **DNS 요청**과 **응답** 사이에 개입하여 데이터를 탈취하거나 수정하는 공격 방식이다.
- **설명**: DNS 응답을 변조하여 사용자가 의도하지 않은 사이트로 연결되게 만든다.

#### DNS 서비스 방해 공격
- **현재 상황**: 지금까지 DNS 서비스에 대한 공격은 많았지만, **성공적으로 DNS 서비스 자체를 방해**하는 공격은 없었다.
